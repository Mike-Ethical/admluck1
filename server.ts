...
if (db.inventory.length === 0 && db.pets.length > 0) {
  ...
  [shadow, bat, frost].filter(Boolean).forEach((pet, idx) => {
    const calculatedVal = calculatePetItemValue(pet.value, 'Normal', true, true);
    db.inventory.push({
...
async function startServer() {
  ...
  app.post('/api/user/roblox/start', async (req, res) => {
    ...
    try {
      ...
      const lookupData = await robloxLookupRes.json();
  ...
  app.post(['/api/matches/:id/join', '/api/matches/join', '/matches/:id/join', '/matches/join'], (req, res) => {
    ...
    if (!Array.isArray(selectedItemIds) || selectedItemIds.length === 0) {
      return res.status(400).json({ error: 'Select pets to match the bet value' });
    }

    if (match.maxJoinerPets && selectedItemIds.length > match.maxJoinerPets) {
      return res.status(400).json({
        error: `This coinflip allows joining with at most ${match.maxJoinerPets} pet${match.maxJoinerPets > 1 ? 's' : ''}. You selected ${selectedItemIds.length}.`
      });
    }

    // Validate opponent items
    ...
    for (const id of selectedItemIds) {
      ...
      if (item.locked) {
        return res.status(400).json({ error: `Item ${item.petName} is currently locked` });
      }
      if (item.totalValue < 1 || item.value < 1) {
        return res.status(400).json({ error: `Item "${item.petName}" has value less than 1.00 (only pets with value 1 or higher accepted in coinflips)` });
      }
    ...
    const minRange = Math.max(1, Math.floor(creatorVal * 0.95));
    const maxRange = Math.ceil(creatorVal * 1.05);

    if (opponentTotalVal < minRange || opponentTotalVal > maxRange) {
      return res.status(400).json({
        error: `Total wagered value (${opponentTotalVal} Val) does not match the required range (${minRange} - ${maxRange} Val).`
      });
    }
    ...
    // Authoritative Server-side outcome computation
  ...
  app.post(['/api/matches/:id/cancel', '/matches/:id/cancel'], (req, res) => {
    ...
    const isOwner = isAuthorizedAdmin(currentUser) || currentUser.id === match.creator.userId;
    if (!isOwner) {
  ...
  app.post(['/api/chat', '/api/chat/send', '/chat', '/chat/send'], (req, res) => {
    ...
    if (trimmedMsg.startsWith('/tip ')) {
      ...
      if (targetUserQuery) {
        let recipient = Array.from(db.users.values()).find(
          (u) =>
        ...
        if (!recipient) {
          recipient = {
            id: `user-${targetUserQuery.toLowerCase().replace(/[^a-z0-9]/g, '')}`,
            username: targetUserQuery,
            robloxUsername: targetUserQuery,
            avatarUrl:
              'https://tr.rbxcdn.com/30DAY-AvatarHeadshot-310966282D3529E36976BF6B07B1DC90-Png/150/150/AvatarHeadshot/Png/isCircular',
            role: 'user',
            verified: true,
            verificationStatus: 'verified',
            totalGames: 0,
  ...
  app.post('/api/tip', (req, res) => {
    ...
    if (!recipient) {
      const cleanName = recipientUsername.trim();
      recipient = {
        id: `user-${cleanName.toLowerCase().replace(/[^a-z0-9]/g, '')}`,
        username: cleanName,
        robloxUsername: cleanName,
        avatarUrl:
          'https://tr.rbxcdn.com/30DAY-AvatarHeadshot-310966282D3529E36976BF6B07B1DC90-Png/150/150/AvatarHeadshot/Png/isCircular',
        role: 'user',
        verified: true,
  ...
  app.get('/api/leaderboard', (_req, res) => {
    const allUsers = Array.from(db.users.values()).filter((u) => u.totalGames > 0);
  ...
  app.post('/api/admin/inventory/add', (req, res) => {
    ...
    const baseVal = customValue !== undefined && customValue !== null ? Math.max(1, Number(customValue)) : (pet ? pet.value : 10);
    const calculatedVal = Math.max(1, calculatePetItemValue(baseVal, variant, fly, ride));
