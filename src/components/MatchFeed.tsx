...
interface MatchFeedProps {
  ...
  onCallBot?: (match: CoinflipMatch) => void;
  onCallBotMatch?: () => void;
}
  ...
  if (matches.length === 0) {
    return (
      <div className="bg-[#0e1422] border border-[#1b263b] rounded-2xl p-10 text-center text-slate-400 font-sans shadow-lg">
        ...
        <p className="text-xs text-slate-500 mt-1">Create a new coinflip or call a test bot!</p>
        {onCallBotMatch && (
          <button
            onClick={onCallBotMatch}
            className="mt-4 px-4 py-2 rounded-xl bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-slate-950 font-gaming font-black text-xs tracking-wider uppercase transition active:scale-95 shadow-[0_0_15px_rgba(245,158,11,0.3)] inline-flex items-center gap-1.5 cursor-pointer"
          >
            <Bot className="w-4 h-4" />
            <span>Call Bot Match (Test)</span>
          </button>
        )}
      </div>
    );
            ...
            <div className="flex items-start justify-between gap-4">
                        ...
                        {(item.mega || item.neon) && (
                          <span
                            className={`absolute -top-1 -right-1 text-[8px] font-black px-1 rounded-full text-white ${
                              item.mega ? 'bg-fuchsia-500' : 'bg-emerald-400 text-slate-950'
                            }`}
                          >
                            {item.mega ? 'M' : 'N'}
                          </span>
              ...
              <div className="text-right shrink-0 flex flex-col items-end justify-start pt-1">
                ...
                <button
                  ...
                  id={`btn-spectate-${match.id}`}
                  className="mt-3 text-slate-400 hover:text-sky-300 text-xs font-gaming font-bold flex items-center gap-1.5 transition-colors cursor-pointer"
                  title="Spectate Coinflip"
                >
              ...
              {isWaiting ? (
                isCreator ? (
                  <div className="flex items-center gap-2">
                    ...
                    {onCallBot && (
                      <button
                        onClick={() => onCallBot(match)}
                        id={`btn-call-bot-${match.id}`}
                        className="py-3 px-4 rounded-xl bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-slate-950 font-gaming font-black text-xs sm:text-sm tracking-wide shadow-[0_0_15px_rgba(245,158,11,0.35)] active:scale-95 transition cursor-pointer flex items-center gap-1.5 shrink-0"
                        title="Summon a test bot to join this match instantly"
                      >
                        <Bot className="w-4 h-4" />
                        <span>Call Bot</span>
                      </button>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {onCallBot && (
                      <button
                        onClick={() => onCallBot(match)}
                        id={`btn-call-bot-${match.id}`}
                        className="py-3 sm:py-3.5 px-3.5 rounded-xl bg-[#162033] hover:bg-[#1f2c45] border border-amber-500/40 hover:border-amber-400 text-amber-300 font-gaming font-black text-xs sm:text-sm tracking-wide transition cursor-pointer active:scale-95 flex items-center gap-1.5 shrink-0 shadow-sm"
                        title="Summon test bot to join this match"
                      >
                        <Bot className="w-4 h-4 text-amber-400" />
                        <span className="hidden xs:inline">Call Bot</span>
                      </button>
                    )}
                  </div>
                )
                ...
                >
                  ...
                  <span>Flipping in Progress • Watch Live</span>
                </button>
              ) : (
                <button
                  onClick={() => onWatchFlip(match)}
