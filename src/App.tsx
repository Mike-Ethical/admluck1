...
export function App() {
  ...
  // Handle Call Bot: instigates a test bot to join a waiting match and immediately launches flip animation
  const handleCallBot = async (match: CoinflipMatch) => {
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json();
        alert(err.error || 'Failed to call bot');
      }
    } catch (err: any) {
      console.error('Call bot error:', err);
    }
  };

  // Handle Call Bot Match: spawns a waiting bot match for testing
  const handleCallBotMatch = async () => {
    try {
      const res = await fetch('/api/matches/spawn-bot-match', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-user-id': currentUser.id,
      ...
      console.error('Spawn bot match error:', err);
    }
  };
